...
| ALB | Internet-facing; default listener forwards to app target group |
| ALB Target Group | Port 80, IP target type, health check `GET /` every 30s |
...
| `HttpListenerArn` | ALB HTTP listener ARN |
| `AppTargetGroupArn` | App target group ARN |
...
16 tests, no AWS credentials required.
